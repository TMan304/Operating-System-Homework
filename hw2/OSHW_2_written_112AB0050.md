...
### 5.25
- a: 對進程長短沒有偏向
- b: 相比a對短進程更友善，能比長進程更早完成作業
- c: 最偏向短進程，短進程能在較高的優先級佇列就完成，相比之下長進程一般會隨著執行過程降低優先級

## Ch6
### 6.7
- a: top和stack的讀存
- b: 在讀取和存取這兩個數據的程式碼關鍵區段，加上mutex或semaphore
### 6.15
- 假設卡死的話，因為禁用中斷的原因，且唯一可運行的核心在執行卡死的程序，OS將無法停止卡死的進程
### 6.18
- acquire不可用時改成將進程放進wait queue並釋放CPU；release多添加呼叫一個位於wait queue的進程